...
def main():
    ...
    for instrNode in root.iter('instruction'):
        ...
        # Sky-, Ice-, and Alder Lake all seem to have load ports 2 + 3
        # based on uops.info / wiki chip, e.g., just check what POP R64 uses:
        # https://uops.info/html-instr/POP_R64.html
        load_port_archs = ["SKL", "ICL", "ADL-P"]
        load_port_list = ["p2", "p3", "p23"]
        skip_instruction = True
        try:
            for arch in instrNode.iter("architecture"):
                if arch.attrib['name'] in load_port_archs:
                    for child in arch:
                        if child.tag == "measurement":
                            if any(port in child.attrib['ports'] for port in load_port_list):
                                print(f"{asm_code} is using loadports")
                                skip_instruction = False
                
        except:
            # we ignore  everything that is not using loadports
            continue
        if skip_instruction:
            continue
        #
    ...
    print("Shuffling lines now...")
    with open(output_fname, "rb") as fd:
        lines = fd.readlines()
    
    # shuffle all lines besides the header
    header = lines[0]
    body = lines[1:]
    random.shuffle(body)
    with open(output_fname, "wb") as fd:
