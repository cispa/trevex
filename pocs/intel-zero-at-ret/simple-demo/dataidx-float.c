...
void init_victim() {
  ...
  victim_data.dummy_str = NULL;
  victim_data.public_str = PUBLIC_STRING;
  victim_data.secret_str = SECRET_STRING;
}
...
void load_glyph(glyph_t* glyphs, 
  ...
  asm volatile (INTELASM(
    ...
    // architectural result: 0x08
    // misspeculated result: 0x10
...
void init() {
  ...
  // we need to create some noise in the beginning for $reason
  system("id");
